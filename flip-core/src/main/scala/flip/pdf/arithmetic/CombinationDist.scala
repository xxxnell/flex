...
trait CombinationDistOps[D[_]<:CombinationDist[_]] extends DistPropOps[D] {
  ...
  def modifyComponent[A](dist: D[A],
                         idx: Int,
                         f: (Double, Dist[A]) => (Double, Dist[A])): D[A] = {
    val (utdWeight, utdDist) = dist.components.asInstanceOf[Components[A]].toList.apply(idx)

    modifyComponents(dist, components =>
      NonEmptyList.fromList(components.toList.updated(idx, f(utdWeight, utdDist))).get
    )
  }
...
object CombinationDist extends CombinationDistOps[CombinationDist] {
  ...
  def modifyRng[A](dist: CombinationDist[A],
                   f: IRng => IRng): CombinationDist[A] = {
    bare(dist.measure, dist.conf, dist.components, f(dist.rng))
  }

  def modifyComponents[A](dist: CombinationDist[A],
                          f: Components[A] => Components[A]): CombinationDist[A] = {
