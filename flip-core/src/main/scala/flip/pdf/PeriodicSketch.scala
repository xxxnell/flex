...
object PeriodicSketch extends PeriodicSketchOps[PeriodicSketch] {
  ...
  def modifyStructure[A](sketch: PeriodicSketch[A], f: Structures => Option[Structures]): Option[PeriodicSketch[A]] =
    sketch match {
      case sketch: AdaPerSketch[A] => AdaPerSketch.modifyStructure(sketch, f)
      case _ =>
        f(sketch.structures).map(utdStrs => bare(sketch.measure, sketch.conf, utdStrs, sketch.thresholds, sketch.count))
    }
  ...
  override def update[A](sketch: PeriodicSketch[A], as: List[(A, Count)]): Option[PeriodicSketch[A]] = sketch match {
