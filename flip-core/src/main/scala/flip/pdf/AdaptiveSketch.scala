...
trait AdaptiveSketchOps[S[_] <: AdaptiveSketch[_]] extends SketchPrimPropOps[S] with AdaptiveSketchLaws[S] { self =>
  ...
  override def count[A](sketch: S[A], start: A, end: A): Option[Count] =
    for {
  ...
  override def narrowUpdate[A](sketch: S[A], as: List[(A, Count)]): Option[S[A]] =
    for {
      (sketch1, old) <- Some(append(sketch, as))
      sketch2 <- super.narrowUpdate(sketch1, old)
    } yield sketch2

  override def rearrange[A](sketch: S[A]): Option[S[A]] =
    for {
...
trait AdaptiveSketchLaws[S[_] <: AdaptiveSketch[_]] { self: AdaptiveSketchOps[S] =>
  ...
  def pdfForQueue[A](sketch: S[A], a: A): Option[Double] =
    for {
...
object AdaptiveSketch extends AdaptiveSketchOps[AdaptiveSketch] {
  ...
  def modifyStructure[A](sketch: AdaptiveSketch[A], f: Structures => Option[Structures]): Option[AdaptiveSketch[A]] =
    sketch match {
      case sketch: AdaPerSketch[A] => AdaPerSketch.modifyStructure(sketch, f)
    }

  def update[A](sketch: AdaptiveSketch[A], as: List[(A, Count)]): Option[AdaptiveSketch[A]] = sketch match {
