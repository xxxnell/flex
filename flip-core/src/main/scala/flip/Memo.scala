...
object Memo extends MemoOps {
  ...
  private case class MemoImpl[K, V](size: Int, queue: mutable.Queue[K], table: mutable.WeakHashMap[K, V]) extends Memo[K, V]

  case object syntax extends MemoSyntax

  def bare[K, V](size: Int, queue: mutable.Queue[K], table: mutable.WeakHashMap[K, V]): Memo[K, V] = MemoImpl(size, queue, table)
