...
import flex.conf.pdf.{CustomAdaPerSketchConf, CustomPeriodicSketchConf, PeriodicSketchConf}
...
class SketchConfSpec extends Specification with ScalaCheck {
  ...
  "ShetchConf" should {
    ...
    "default" in {
      implicitly[flex.conf.pdf.SketchConf] must beAnInstanceOf[flex.conf.pdf.SketchConf]
    }
    ...
    "customize locally" in {
      ...
      implicitly[flex.conf.pdf.SketchConf] must equalTo(conf)
    }
