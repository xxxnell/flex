...
trait SketchSyntax extends SketchSyntax1 {
  ...
  implicit class SketchMonadSyntaxImpl[A](sketch: Sketch[A]) {
    def map[B](
      f: A => B
    )(implicit functor: SketchFunctor[Sketch, Sketch, SketchConf], measureB: Measure[B], conf: SketchConf): Sketch[B] =
      functor.map(sketch, f, measureB, conf)
    def flatMap[B](
      f: A => Dist[B]
    )(implicit bind: SketchBind[Sketch, Dist, Sketch, SketchConf], measureB: Measure[B], conf: SketchConf): Sketch[B] =
