...
object ConfExp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val confs: List[SketchConf] = Params.toConfs
    val normal = (idx: Int) => NumericDist.normal(0.0, 1, IRng(idx))
    val incr = (idx: Int) => NumericDist.normal(idx * 0.1, 1, IRng(idx))
    ...
    val normalOut = normalMeanKlds.map { case (conf, kld) => conf2Str(conf) + s", $kld" }.mkString("\n")
    val incrDriftOut = incrDriftMeanKlds.map { case (conf, kld) => conf2Str(conf) + s", $kld" }.mkString("\n")
