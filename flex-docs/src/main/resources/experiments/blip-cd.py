...
name = "blip-cd-normal"
dir = "../../../flex-bench/experiments/blip-cd-normal/"
...
def pdf_data_loc(i):
    return dir + "blip-cd-normal-pdf-" + str(i) + ".out"

def cdf_data_loc(i):
    return dir + "blip-cd-normal-cdf-" + str(i) + ".out"

def delta_data_loc(i):
    return dir + "blip-cd-normal-delta-" + str(i) + ".out"

def pdf_expected(x, start, duration, i):
    if i <= start:
        return norm.pdf(x)
    elif i <= start + duration:
        return norm.pdf(np.array(x) - dest)
    else:
        return norm.pdf(x)

def cdf_expected(x, start, duration, i):
    if i <= start:
        return norm.cdf(x)
    elif i <= start + duration:
...
median_data_loc = dir + "blip-cd-normal-median.out"
...
kld_data_loc = dir + "blip-cd-normal-kld.out"
...
ed_data_loc = dir + "blip-cd-normal-ed.out"
...
axed.axvline(moving_start + moving_duration, color='r', linestyle=':', linewidth=1)
...
utd_animation2 = pdfplt.animated_pdfplot_bar(
  pdf_data_loc, countmin, countmax, step, lambda x, i: pdf_expected(x, moving_start, moving_duration, i), xmin, xmax, ymin, ymax)
...
cdf_animation = cdfplt.animated_cdfplot(
  cdf_data_loc, delta_data_loc, start, end, step, lambda x, i: cdf_expected(x, moving_start, moving_duration, i), xmin, xmax, 0, 1)
