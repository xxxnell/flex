...
object RecurSketch extends RecurSketchOps[RecurSketch] {
  ...
  def apply[A](measure: Measure[A],
    ...
    bare(measure, structure, conf, thresholds)

  def bare[A](measure: Measure[A],
              structure: Structures,
              conf: SketchConf,
              thresholds: Stream[Double]): RecurSketch[A] =
    RecurSketchImpl(measure, structure, conf, thresholds)

  def empty[A](implicit measure: Measure[A], conf: SketchConf): RecurSketch[A] = cont(measure, conf)

  def cont[A](measure: Measure[A], conf: SketchConf): RecurSketch[A] = PeriodicSketch.empty(measure, conf)
